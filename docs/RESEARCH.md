...
| Decontamination against held-out evaluation prompts | Standard practice; DriftLock prompts are the held-out set | `is_contaminated()`, applied to every record |
| Cases where the user is *right* and the agent updates | Guards against teaching reflexive contrarianism in place of sycophancy (Sharma et al., 2023) | Seed corpus, `sycophancy` and `honesty` categories |
