...
class TestGrazer:
    ...
    def test_discover_content_no_grazer(self, bus_with_agent, monkeypatch):
        """Without grazer package, returns empty list gracefully."""
        monkeypatch.setitem(sys.modules, "grazer", None)
        bus_with_agent.bind_platforms("testbot", ["moltbook"])
        results = bus_with_agent.discover_content("testbot", topic="AI")
        assert results == []

    def test_discover_content_uses_per_platform_api(self, bus_with_agent, monkeypatch):
        """grazer-skill exposes discover_<platform>(); unknown/failing ones are skipped."""

        class FakeGrazerClient:
            def discover_moltbook(self, limit=10):
                return [{"id": i, "platform": "moltbook"} for i in range(limit)]

            def discover_bottube(self, limit=10):
                raise ConnectionError("offline")

        monkeypatch.setitem(
            sys.modules, "grazer", types.SimpleNamespace(GrazerClient=FakeGrazerClient)
        )
        bus_with_agent.bind_platforms("testbot", ["bottube", "twitter", "moltbook"])
        results = bus_with_agent.discover_content("testbot", limit=3)
        assert [r["platform"] for r in results] == ["moltbook"] * 3
